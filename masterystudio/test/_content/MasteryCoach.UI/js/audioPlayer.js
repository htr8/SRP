...
function notifyEnded() {
    // CATCH the interop rejection: this module keeps its own app-lifetime AudioContext + state, so an
    // OnPlaybackEnded fired around a source switch / dispose can hit a transiently-unresolvable
    // JsAudioPlayer ("no tracked object with id N"); left unobserved that rejection became a .NET
    // unhandled error that tore down the whole circuit (matches the stems-engine crash, same class).
    // The end notification is best-effort — swallow the fault. (clearEndedCallback nulls state.dotnet
    // before dispose; this guards the transient window before that runs.)
    if (state.dotnet) {
...
async function stretchLoad(arrayBuffer, key) {
    ...
    // Pre-bucket the waveform once at a fine resolution; getPeaks() resamples from this cheaply. The
    // cache resolution is the HARD ceiling on zoomed-in detail (getPeaks upsamples from it), so keep it
    // high: ~200 buckets/sec means even a max-zoom 1-2s window still has hundreds of source samples to
    // draw instead of a handful smeared into chunky blocks (user report). computePeaks is a single linear
    // pass over the PCM regardless of bucket count, so this costs ~no extra time — only memory (a 48k-cap
    // song is ~576KB across the 3 Float32 arrays), which is trivial and freed with the record.
    const targetBuckets = Math.min(48000, Math.max(2000, Math.floor(decoded.duration * 200)));
