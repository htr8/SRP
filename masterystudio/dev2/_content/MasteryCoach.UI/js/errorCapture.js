...
(function () {
    ...
    // Service-worker errors run in a separate global and cannot reach the window error hooks above.
    // The COI worker reports failed fetches explicitly so a broken module load is visible both in the
    // error bar and in Diagnostics instead of being converted into an empty successful response.
    if (navigator.serviceWorker) {
        navigator.serviceWorker.addEventListener('message', function (e) {
            var data = e.data;
            if (!data || data.type !== 'mc-service-worker-fetch-failed') return;
            var detail = 'Service-worker fetch failed for ' + (data.url || 'an unknown resource') +
                ': ' + (data.message || 'unknown failure');
            show(detail);
            report('error', 'service-worker', detail);
        });
    }
